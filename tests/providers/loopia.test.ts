...
describe('loopia', () => {
  ...
  describe('getRecords', () => {
    it('fetches records for a subdomain', async () => {
      ...
      expect(callBody).toContain('<string>example.com</string>');
      expect(callBody).toContain('<string>rm</string>');
    });

    it('uses @ for root domain records', async () => {
      mockFetch.mockResolvedValueOnce(xmlRpcArray([]));

      const provider = loopia({ username: 'user', password: 'pass', domain: 'example.com' });
      await provider.getRecords('example.com');

      const callBody = mockFetch.mock.calls[0]![1].body as string;
      expect(callBody).toContain('<string>@</string>');
    });
  ...
  describe('createRecord', () => {
    it('creates a record and returns the result', async () => {
      ...
      const addBody = mockFetch.mock.calls[0]![1].body as string;
      expect(addBody).toContain('<methodName>addZoneRecord</methodName>');
      expect(addBody).toContain('<string>example.com</string>');
      expect(addBody).toContain('<string>rm</string>');
      expect(addBody).toContain('<name>type</name>');
      expect(addBody).toContain('<string>CNAME</string>');
    });
    ...
    it('throws when created record cannot be found', async () => {
      mockFetch.mockResolvedValueOnce(xmlRpcOk('OK'));
      mockFetch.mockResolvedValueOnce(xmlRpcArray([]));
      ...
      ).rejects.toThrow('Loopia: record was created but could not be found');
    });
  });
  ...
  describe('deleteRecord', () => {
    ...
    it('throws when removeZoneRecord returns non-OK', async () => {
      mockFetch.mockResolvedValueOnce(xmlRpcOk('UNKNOWN_ERROR'));

      const provider = loopia({ username: 'user', password: 'pass', domain: 'example.com' });
      await expect(provider.deleteRecord('rm:101')).rejects.toThrow(
        'Loopia: removeZoneRecord failed: UNKNOWN_ERROR'
      );
    });
